...
$DOM_ = getDomain( $HOST_ );

echo " // DOMAIN: " . $DOM_ . "
";

$REF_TEST = $this->agent->referrer();
...
if ( $SESSID == "" ) {
    ...
    $session_id = $this->session->userdata( 'SESSID' );
}
...
if ( $this->agent->is_mobile() ) {
    $mobile = "Yes " . $mobile[ 1 ] . " " . $mobile[ 2 ] . " " . $mobile[ 3 ] . " " . $this->agent->mobile();
    $isMobile = 1;
} else {
    $mobile = "No";
    $isMobile = 0;
...
if ( isset( $_GET[ 'campaign_id' ] ) || isset( $_GET[ 'url_campaign_id' ] ) || $campaign_id != "" ) {
    ...
    $check_campaign_ = $this->db->query( "SELECT *,a.status as a_status,a.active as a_active FROM campaigns as c JOIN accounts as a ON (c.account_id = a.account_id) WHERE campaign_id='$pgCampaign_id' and a.active='1' and c.active='1'" );
    if ( $check_campaign_->num_rows() == 1 ) {
        $have_Pgcampaign = $check_campaign_->row();
    }

    if ( $check_campaign_->num_rows() == 1 && $check_campaign->num_rows() == 1 ) {
            ...
            if ( $num_rows > 0 ) {
                ...
                if ( $ip_rows < 1 ) {
                    ...
                    if ( $entry == 0 ) {
                        ...
                        $page_time = mdate( "%Y-%m-%d %H:%i:%s", time() );
                        $last_page = $this->agent->referrer() . " : " . $page_time . ";" . $have_ip->last_page;
