<?php
	...
	if( isset($_SESSION['page_views']) && $_SESSION['page_views'] < 5){
		
		

	
       if( $have_campaign->location_tracking == 1){
		...
		function getLocation()
		  ...
		  if (navigator.geolocation)
			...
			function geoSuccess(p) {
				...
				jQuery.get("'. site_url('incomingdata/saveLatLng') .'?rand='. rand(100,500) .'",{ip:"'. $visitor.'",lat:lat,lng:lng,campaign_id:"'. $campaign_id .'"},function(info){
			...
			function showError(error)
		  {
			...
			jQuery.post("'. site_url('incomingdata/saveLatLng') .'?rand='. rand(100,500) .'",{ip:"'. $visitor.'",geoErr:geoError,campaign_id:"'. $campaign_id .'"},function(info){
			...
			function showPosition(position)
		  {
			...
			jQuery.post("'. site_url('incomingdata/saveLatLng') .'?rand='. rand(100,500) .'",{ip:"'. $visitor.'",lat:lat,lng:lng,campaign_id:"'. $campaign_id .'"},function(info){
