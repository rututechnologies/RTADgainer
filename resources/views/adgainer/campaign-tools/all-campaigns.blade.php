...
<div class="wrapper border-bottom white-bg page-heading">
    ...
    <ol class="breadcrumb" style="padding-bottom: 20px;">
        ...
        <li class="active"><strong>All Campaigns</strong></li>
    </ol>
</div>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label>Account :</label>
                                    <select id="selectAccount" name="account_id" class="form-control" required>
                                        <option value="">Select An Account</option>
                                        @foreach($accounts as $account)
                                        <option {{ ($account_id === $account->account_id) ? 'selected' : '' }} value="{{$account->account_id}}" data-timezone="{{$account->account_time_zone}}">{{$account->accountName}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    (function($){
        $(function( ){
            ...
            $('#selectAccount').change(function(){
                $('#selectTimeZone').val($(this).find(':selected').data('timezone'));
            });
        });
    }(jQuery));
</script>
