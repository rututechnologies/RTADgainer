...
$update_phones = mysqli_query(
    $conn, "UPDATE 
	phone_number_inventory 
	SET 
	active='0' 
	WHERE 
	(campaign_id='$campaign_id' and timestamp < '$time_before_current')"
);
$time5 .= " 1 ";
////////////////////////
/////////////get active visitor info
echo "
LINE:  " . __LINE__;
...
LINE:  " . __LINE__;
            ...
            if ( $v_id_ != FALSE && is_numeric( $v_id_ ) ) {
                $updateRow = updateVisitorRow( $duration, $mp3_name, $callid, $caller_state, $caller_country, $calls, $call_status, $time_of_call, $phonenumber, $campaign_id, $last_row_, $unique_call_id, $v_id_, __LINE__ );
            } else {
                $keyword = getKeywordType( $have_campaign->country_tracking, $have_last_id_query->$keywordType );
                $mp3_name = getMP3name( $recordings );
                $time_stamp = date( "Y-m-d H:i:s", (time() - 20 ) );
...
LINE:  " . __LINE__;
            $time5 .= " 13 ";
            $campaign_PH = getCampaignIdWithPhone( $phonenumber, 'phone_number_inventory', 'phone_number' );
...
LINE:  " . __LINE__;
        ...
        $time_of_call = $callstart;
        $call_status = getCallTimeAndRecording( $calls, $recordings, $duration );
        //mail('hayashi@adgainer.co.jp','jpn call',13);
        $time5 .= " 16 ";
    } else {
        echo "
LINE:  " . __LINE__;
        //mail('hayashi@adgainer.co.jp','jpn call',14);
        $calls = 1 + getTotalActiveCallsMade( $callid, $phonenumber, $visitor );
        $time_of_call = $callstart;
        $keywordType = $have_campaign->country_tracking;
        $call_status = getCallTimeAndRecording( $calls, $recordings, $duration );
