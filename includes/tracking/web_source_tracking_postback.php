...
if ( ! $have_active_visitor_data ) {
    ...
    if ( $have_last_id_query ) {
        ...
        if ( $calls > 1 ) {
            ...
            addNewCall( $have_campaign->account_id, $duration, $phonenumber, $keywordType, $keyword, $have_last_id_query->matchtype, $have_last_id_query->traffic_type, $mp3_name, $callid, $caller_state, $caller_country, $have_last_id_query->source, $calls, $call_status, $time_of_call, $campaign_id, $time_stamp, $unique_call_id, $jpn_time_stamp );
        } else {
            $time5 .= " 10 ";
            $mp3_name = getMP3name( $recordings );
        ...
        if ( isset( $last_phone_data->campaign_id ) ) {
            ...
            $time5 .= " 13 ";
            $campaign_PH = getCampaignIdWithPhone( $phonenumber, 'source_number_inventory', 'phone_number' );
