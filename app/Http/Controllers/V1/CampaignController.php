    ...
    public function new_Campaign_General(Request $request)
    {
         try
         {
                ...
                $newcampaigns->goal_notify_email ="";
                 ...
                 $newcampaigns->notified_date=date_create();
                 $newcampaigns->text_notify ="1";
                 ...
                 foreach($request->input("postback_fields")as $postback_fields)
                {
                 ...
                 $newcampaigns->heatmap_code ="1";
                 $newcampaigns->fingerprint_code ="1";
                 $newcampaigns->tracking_code ="1";
                 $newcampaigns->email_code="1"; 
                 $newcampaigns->multi_code="1";   
                 $newcampaigns->source_code="1";  
                 $newcampaigns->source_numbers ="1";  
                 $newcampaigns->source_multi="1";  
                 $newcampaigns->omit_google_ip="1";
                 $newcampaigns->email_tracking = $request->email_tracking; 
                 $newcampaigns->email_tracking_email =""; 
                 $newcampaigns->email_tracking_post_page =""; 
                 $newcampaigns->active =""; 
                 $newcampaigns->number_serve_active=$request->number_serve_active;
                 $newcampaigns->number_serve_schedule=""; 
...
//               $newcampaigns->device_tracking  ="KDM"; 
                ...
                $newcampaigns->camp_custom1=$request->camp_custom1;
                 ...
                 $newcampaigns->sttCount="1";
                $newcampaigns->TWILIO_XML=""; 
                 $save = $newcampaigns->save();
                // echo($save);
                 if($save=='1')
                 {
                    echo "<script type='text/javascript'>confirm('Are you Sure To Submit this') </script>";
                     DB::commit();
                    return view( $this->view_directory_name .'micros.newcampaign' );
                 }
